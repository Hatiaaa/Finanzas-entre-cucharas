...
export const TransactionForm: React.FC<TransactionFormProps> = ({ accounts, categories, onAddTransaction }) => {
  ...
  return (
    <div className="max-w-4xl mx-auto bg-[#1c1c24] rounded-3xl shadow-lg border border-gray-800 overflow-hidden animate-fade-in">
      <div className="flex border-b border-gray-800">
        <button
          onClick={() => setMode('standard')}
          className={`flex-1 py-4 text-center font-medium transition-colors ${
            mode === 'standard' ? 'bg-[#6c5dd3]/10 text-[#6c5dd3] border-b-2 border-[#6c5dd3]' : 'text-gray-500 hover:bg-white/5'
          }`}
        >
        ...
        <button
          ...
          className={`flex-1 py-4 text-center font-medium transition-colors ${
            mode === 'transfer' ? 'bg-[#3b82f6]/10 text-[#3b82f6] border-b-2 border-[#3b82f6]' : 'text-gray-500 hover:bg-white/5'
          }`}
        >
          <span className="flex items-center justify-center gap-2">
            <RefreshCw size={18} />
      ...
      <form onSubmit={handleSubmit} className="p-8 space-y-6">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div>
            ...
            <div className="relative">
              ...
              <input
                ...
                className="w-full rounded-xl bg-[#13131a] border border-gray-700 text-white pl-8 p-3 focus:ring-1 focus:ring-[#6c5dd3] focus:border-[#6c5dd3] outline-none placeholder-gray-600"
                placeholder="0.00"
              />
            </div>
          </div>
            ...
            <>
              <div>
                ...
                <select
                  value={accountId}
                  onChange={(e) => setAccountId(e.target.value)}
                  className="w-full rounded-xl bg-[#13131a] border border-gray-700 text-white p-3 focus:ring-1 focus:ring-blue-500 focus:border-blue-500 outline-none"
                >
                  {accounts.map(acc => <option key={acc.id} value={acc.id}>{acc.name}</option>)}
                </select>
              </div>
          ...
          <div className="md:col-span-2">
            ...
            <input
              ...
              className="w-full rounded-xl bg-[#13131a] border border-gray-700 text-white p-3 focus:ring-1 focus:ring-[#6c5dd3] focus:border-[#6c5dd3] outline-none placeholder-gray-600"
          ...
          <div className="md:col-span-2">
            ...
            <div className="flex items-center justify-center w-full">
              <label className="flex flex-col items-center justify-center w-full h-32 border-2 border-gray-700 border-dashed rounded-xl cursor-pointer bg-[#13131a] hover:bg-gray-800 transition-colors">
                <div className="flex flex-col items-center justify-center pt-5 pb-6">
                  <Upload className="w-8 h-8 mb-4 text-gray-500" />
                  <p className="mb-2 text-sm text-gray-500"><span className="font-semibold">Click para subir</span></p>
                  <p className="text-xs text-gray-600">PNG, JPG o PDF</p>
                </div>
                <input type="file" className="hidden" onChange={(e) => setFile(e.target.files ? e.target.files[0] : null)} />
              </label>
            </div>
            {file && <p className="text-sm text-[#6c5dd3] mt-2 text-center">Archivo seleccionado: {file.name}</p>}
          </div>
        ...
        <div className="flex justify-end">
          <button
            type="submit"
            className={`px-8 py-3 text-white font-bold rounded-xl shadow-lg hover:shadow-xl transition-all ${
              mode === 'standard' ? 'bg-[#6c5dd3] hover:bg-[#5b4eb8]' : 'bg-[#3b82f6] hover:bg-[#2563eb]'
            }`}
          >
